...
class Tests(unittest.TestCase):
    ...
    def test_minpoly(self):
        minpoly = gf.minpoly(np.array([0b10]), gf.gen_pow_matrix(0b1011))

        assert_array_equal([1, 0, 1, 1], minpoly[0])
        assert_array_equal([2, 4, 6], minpoly[1])

    def test_minpoly_2(self):
        minpoly = gf.minpoly(np.array([0, 0b10]), gf.gen_pow_matrix(19))
    ...
    def test_polyprod_normalize_2(self):
        pm = gf.gen_pow_matrix(19)
        p1 = np.array([pm[-3, 1], pm[-1, 1]])
        zero = np.array([0, 0])
        assert_array_equal(gf.polyprod(p1, zero, pm), [0])

    def test_polyprod_normalize_3(self):
        pm = gf.gen_pow_matrix(19)
        p1 = np.array([0, pm[-3, 1], pm[-1, 1]])
        zero = np.array([0])
        assert_array_equal(gf.polyprod(p1, zero, pm), [0])
    ...
    def test_polydiv(self):
        div = gf.polydiv(np.array([0b10, 0b1]), np.array([0b1]), gf.gen_pow_matrix(0b1011))
        assert_array_equal(div[0], [0b10, 0b1])
        assert_array_equal(div[1], [0])

    '''def test_polydiv_normalize(self):
        div = gf.polydiv(np.array([0, 0b10, 0b1]), np.array([0b1]), gf.gen_pow_matrix(0b1011))
        assert_array_equal(div[0], [0b10, 0b1])
        assert_array_equal(div[1], [0b0])

    def test_polydiv_normalize_2(self):
        div = gf.polydiv(np.array([0b10, 0b1]), np.array([0, 0b1]), gf.gen_pow_matrix(0b1011))
        assert_array_equal(div[0], [0b10, 0b1])
        assert_array_equal(div[1], [0b0])

    def test_polydiv_normalize_3(self):
        div = gf.polydiv(np.array([0, 0b10, 0b1]), np.array([0, 0b1]), gf.gen_pow_matrix(0b1011))
        assert_array_equal(div[0], [0b10, 0b1])
        assert_array_equal(div[1], [0b0])'''

    def test_polydiv_2(self):
        div = gf.polydiv(np.array([0b10, 0b1]), np.array([0b10]), gf.gen_pow_matrix(0b1011))
        assert_array_equal(div[0], [0b1, 0b101])
        assert_array_equal(div[1], [0])

    def test_polydiv_3(self):
        div = gf.polydiv(np.array([0b10, 0b1]), np.array([0b10, 0b0]), gf.gen_pow_matrix(0b1011))
    ...
    def test_polydiv_zero(self):
        ...
        for elem in pm[:, 1]:
            self.assertRaises(BaseException, lambda: gf.polydiv(np.array([elem]), np.array([0]), pm))

    def test_polyprod_and_polydiv(self):
        pm = gf.gen_pow_matrix(108439)
        p1 = np.array([pm[5, 1], pm[3, 1]])
        p2 = np.array([pm[2, 1], pm[-1, 1]])
        self.do_polyprod_and_polydiv_test(p1, p2, pm)

    def test_polyprod_and_polydiv_2(self):
        pm = gf.gen_pow_matrix(76553)
        p1 = np.array([pm[5, 1], pm[9, 1]])
        p2 = np.array([pm[6, 1], pm[-2, 1]])
        self.do_polyprod_and_polydiv_test(p1, p2, pm)
    ...
    def test_divide_zero(self):
        ...
        self.assertRaises(BaseException, lambda: gf.divide(np.array([[pm[-1, 1]]]), np.array([[0]]), pm))

    def test_divide_zero_2(self):
        pm = gf.gen_pow_matrix(10187)
        for elem in pm[:, 1]:
            self.assertEqual(0, gf.divide(np.array([[0]]), np.array([[elem]]), pm))

    def test_divide_itself(self):
        pm = gf.gen_pow_matrix(54193)
        for elem in pm[:, 1]:
            self.assertEqual(1, gf.divide(np.array([[elem]]), np.array([[elem]]), pm))
    ...
    def test_euclid(self):
        ...
        assert_array_equal(right_answer[0], result[0])
        assert_array_equal(right_answer[1], result[1])
        assert_array_equal(right_answer[2], result[2])
    ...
    def test_linsolve(self):
        ...
        self.assertTrue(gf.linsolve(A, np.array([pm[1, 1], pm[5, 1], pm[-3, 1]]), pm) is np.nan)
    ...
    def test_linsolve_3(self):
        ...
        A = np.array([[pm[5, 1], pm[20, 1], pm[6, 1]],
             ...
             [pm[-1, 1], pm[2, 1], pm[9, 1]]])
        assert_array_equal([3009, 23136, 63822], gf.linsolve(A, np.array([pm[1, 1], pm[5, 1], pm[-3, 1]]), pm))
    ...
    def test_linsolve_6(self):
        ...
        self.assertTrue(gf.linsolve(A, np.array([pm[67, 1], pm[-39, 1], pm[87, 1], pm[49, 1]]), pm) is np.nan)
    ...
    def test_linsolve_random(self):
        while True:
            ...
            pm_len = len(pm)
            n = 50
            A = np.array([[pm[random.randint(0, pm_len - 1), 1] for _ in range(n)] for _ in range(n)])
            b = np.array([pm[random.randint(0, pm_len - 1), 1] for _ in range(n)])
            solution = gf.linsolve(A, b, pm)
            if not (solution is np.nan):
                subst = [gf.sum(gf.prod(A[i].reshape(A[i].size, 1), solution.reshape(solution.size, 1), pm))[0][0] for i in range(n)]
